...
describe('HybridWebCache', () => {
	...
	it('should retrieve all valid cached items when getAll is called', async () => {
		...
		expect(result).toEqual(new Map([['key1', { value: 'value2', expiresAt: expect.any(Number), isExpired: false }]]));
	...
	it('should deal with complex objects', async () => {
		...
		expect(all).toEqual(new Map([['complex', { value: { name: 'square', enabled: true }, expiresAt: expect.any(Number), isExpired: false }]]));
